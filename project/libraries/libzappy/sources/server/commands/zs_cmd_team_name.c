...
** Last update Sat Jul 12 14:00:22 2014 raphael defreitas
*/
...
static bool	string_finder(const char *data, const char *searched)
{
  if (data == NULL || searched == NULL)
    return (false);
  return (strcmp(data, searched) == 0);
}
...
bool		zs_cmd_team_name(t_zs *this, t_zc *zc, const char *cmd)
{
  ...
  if ((team_name = list_find(this->team_names, &string_finder, cmd, IT_DATA)) == NULL)
    return (false);
